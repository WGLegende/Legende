...
public class inventory_actions : MonoBehaviour
{
    ...
    public void create_action_on_inventory_object(inventory_object obj, inventory_slot_master slot_master){
        switch(obj._type_object){
            case enum_manager.type_object.equipement : equipe_object(obj, slot_master);
                break;
            case enum_manager.type_object.consommable : consomme_object(obj);
                break;
            case enum_manager.type_object.ressource : // nothing needs to happen so far...
                break;
            case enum_manager.type_object.relique : equipe_relique(obj);
                break;
            case enum_manager.type_object.savoir : affiche_savoir(obj);
                break;
            case enum_manager.type_object.plan : affiche_plan(obj);
                break;
            case enum_manager.type_object.quete : affiche_quete(obj);
                break;
            case enum_manager.type_object.carte : affiche_carte(obj);
                break;
        }
    }
    ...
    public void equipe_object(inventory_object obj, inventory_slot_master slot_master){
        ...
        inventory_object currently_equiped = inventory_objects_manager.instance.object_list.FirstOrDefault(o => 
                                                o._type_object == obj._type_object && 
                                                o._type_equipement == obj._type_equipement && 
                                                o.is_equiped
                                            );
        if(currently_equiped != null){
            currently_equiped.is_equiped = false;

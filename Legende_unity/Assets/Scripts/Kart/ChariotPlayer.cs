...
public class ChariotPlayer : MonoBehaviour
{
    ...
    void Update(){
        ...
        if (hinput.anyGamepad.leftTrigger.pressed ){ // Frein
      ...
      // PHYSICS CHARIOT
        ...
        if(Mathf.RoundToInt(vitesse_actuelle) > 1){

        SplineFollow.Speed = Mathf.RoundToInt(vitesse_actuelle);
        print("SplineFollow :"+ SplineFollow.Speed);
        }
        else{
        ...
        if(hinput.anyGamepad.Y.justPressed && VapeurBar.instance.useVapeur(10f)){ // Jump 
